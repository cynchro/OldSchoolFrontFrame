...
The **clientes** example loads rows from a public demo API (`getClientes()` in `services/`), stores them in `ctx.state`, and renders a **table by hand** (`createElement` + `textContent`)—no list helpers, no magic.
...
Fetch in the service, assign in the module, render however you like (here is a **conceptual** table sketch—`data-repeat` is **not** implemented; it only illustrates how you might think about rows):

```html
<table>
  <tbody>
    <!-- Conceptual: repeat rows from ctx.state.rows -->
    <tr data-repeat="rows">
      <td data-bind="id"></td>
...
In this repo, the **clientes** example builds table rows with plain `document.createElement` after `getClientes()` runs—explicit and easy to debug. See `example/modules/clientes/`.
